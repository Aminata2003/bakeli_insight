...
ALIASES: dict[str, list[str]] = {
    ...
    "satisfaction_qualitative": [
        "satisfait du coaching", "qualite du coaching", "qualite du coach",
        "evaluation du coach", "satisfaction", "niveau de satisfaction",
    ],
...
# Garde-fou : si la colonne "coach" contient en réalité une réponse de
# qualité (mauvais mapping d'un fichier source, ex: "Qualité du coaching"),
# on préfère ignorer la valeur plutôt que d'enregistrer un faux nom de coach.
MOTS_NON_COACH: set[str] = {
    "bonne", "bon", "moyenne", "moyen", "excellente", "excellent",
    "mauvaise", "mauvais", "insuffisante", "insuffisant",
    "tres bonne", "tres bon", "tres satisfait", "satisfait",
    "peu satisfait", "insatisfait", "oui", "non",
}


def _est_nom_coach_plausible(valeur: str) -> bool:
    return normaliser(valeur) not in MOTS_NON_COACH
...
def transformer_ligne(ligne: dict, mapping: dict[str, str | None], index: int, nom_fichier: str) -> dict:
    ...
    coach_brut = valeur_pour("coach")
    coach = str(coach_brut) if coach_brut and _est_nom_coach_plausible(str(coach_brut)) else None
